...
- (UIImage *)trackImage
{
    if(_trackImage==nil)
    {
        ...
        UIView *bgView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 3, 2)];
        bgView.backgroundColor = [UIColor redColor];
        UIImage *viewImage = [UIImage imageWithView:bgView];
        image = [image resizableImageWithCapInsets:UIEdgeInsetsMake(0.0, 2.0, 0.0, 2.0)];
...
- (UIImage *)lowerHandleImageNormal
{
    if(_lowerHandleImageNormal==nil)
    {
        ...
        {
            UIImage *image = [self imageFromBundle:@"settings_btn_point"];
...
- (CGRect)trackRect
{
    ...
    retValue.size = CGSizeMake(currentTrackImage.size.width, 1);
    ...
    retValue.origin = CGPointMake(xLowerValue - 4, (self.bounds.size.height/2.0f) - (retValue.size.height/2.0f));
    retValue.size.width = xUpperValue-xLowerValue + 8;
...
{
    [self trackBackgroundImage];
    CGRect trackBackgroundRect;
    
    trackBackgroundRect.size = CGSizeMake(_trackBackgroundImage.size.width, 1);
...
- (CGRect)thumbRectForValue:(float)value image:(UIImage*) thumbImage
{
    ...
    thumbRect.origin = CGPointMake(xValue, (self.bounds.size.height/2.0f) - (thumbRect.size.height/2.0f) - 1);
...
- (void) addSubviews
{
    ...
    //------------------------------
    // Track
    UIImage *abc = [self trackImageForCurrentValues];
    //self.track = [[UIImageView alloc] initWithImage:[self trackImageForCurrentValues]];
    self.track = [[UIImageView alloc] init];
    self.track.backgroundColor = [UIColor colorWithRed:236/255.0 green:82/255.0 blue:66/255.0 alpha:1.0];
    ...
    self.trackBackground.backgroundColor = [UIColor colorWithRed:222/255.0 green:222/255.0 blue:222/255.0 alpha:1.0];
    //self.trackBackground.backgroundColor = [UIColor blackColor];
