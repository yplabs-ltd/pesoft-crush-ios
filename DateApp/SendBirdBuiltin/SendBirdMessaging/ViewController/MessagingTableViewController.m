...
// by yjkim
- (void)removeMsgBlockUnBlock
{
    for (int i=0;i<[messageArray count]; i++) {
        if ([[messageArray objectAtIndex:i] isKindOfClass:[SendBirdMessage class]]) {
            SendBirdMessage *message = (SendBirdMessage *)[messageArray objectAtIndex:i];
            NSString *msg = [message message];
            
            if ([msg isEqual: @"MSG_BLOCK"]) {
                message.message = @"[매칭이 성공하였습니다.]";
            } else if ([msg isEqual: @"MSG_UNBLOCK"]) {
                message.message = @"[대화방이 생성되었습니다.]";
            }
        }
    }
}
...
{
    [SendBird endMessagingWithChannelUrl:self.channelUrl];
    /* yjkim
...
- (void)viewDidLoad {
    ...
    //by yjkim, [self.navigationController.navigationBar setTitleTextAttributes:@{NSForegroundColorAttributeName : [UIColor whiteColor]}];
...
// by yjkim
- (void) notifyUpdatedMessagingChannel:(SendBirdMessagingChannel *) channel
{
    if (viewMode == kMessagingViewMode) {
...
- (void) startChatting
{
    ...
    /* by yjkim
    [SendBird igawLoginWithUserId:self.userId andUserName:self.userName andUserImageUrl:@"" andAccessToken:@""];
    ...
    if (viewMode == kMessagingMemberViewMode) {
        ...
        // by yjkim, (join 은 밖에서 함)
        //[self startMessagingWithUser:self.targetUserId];
        //[SendBird joinMessagingWithChannelUrl:self.channelUrl];
        //[SendBird markAsRead];
    }
}
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    // by yjkim
    viewMode = kMessagingChannelListViewMode;
    [SendBird leaveChannel:self.channelUrl];
    ...
    if (scrollView == self.channelMemberListTableView) {
        ...
        if (scrollView.contentOffset.y < 0 && endDragging == YES) {
            [[SendBird queryMessageListInChannel:[SendBird getChannelUrl]] prevWithMessageTs:mMinMessageTs andLimit:30 resultBlock:^(NSMutableArray *queryResult) {
                ...
                [self removeMsgBlockUnBlock];   // yjkim
                [self.tableView reloadData];
...
// yjkim
#pragma mark - Jiver setEventHandlerConnectBlock
...
- (void)allDataReceivedBlock:(NSUInteger)jiverDataType withCount: (int) count
{
    if (viewMode == kMessagingViewMode) {
        if (jiverDataType == SendBirdDataTypeMessage) {
            [self scrollToBottomWithReloading:YES force:NO animated:NO];
        }
    }
}
